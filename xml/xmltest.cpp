...
int example_3()
{
	...
	XMLElement *temp = doc.FirstChildElement();
	for (; temp; temp = temp->NextSiblingElement()) {
		printf("name: %s\n", temp->Name());
	}

	printf("NoChildren? %d\n", doc.FirstChildElement( "PLAY" )->NoChildren());

	// 遍历第一个PLAY下的节点
	XMLElement *curr = doc.FirstChildElement("PLAY")->FirstChildElement();
	for (; curr; curr = curr->NextSiblingElement()) {
		printf("name: %s, %s\n", curr->Name(), curr->GetText());
	}
