...
int main()
{
    ...
    string json_document = "{\"age\" : 123, \"name\" : \"wen\"}";
    ...
    // 对文件解析
    cout << "************************\n";
    ifstream ifs("style_writer.json", ios::binary);
    ...
    {
        cout << "null: " << json_object["null"].isNull() << endl;
        cout << "age:" << json_object["age"].asInt() << endl;
        int num = json_object["array"].size();
        for (int i = 0; i < num; i++)
        {
            cout << json_object["array"][i].asString() << " ";
        }
        ifs.close();
        cout << endl;
    }
