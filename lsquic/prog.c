...
int prog_init(struct prog *prog, unsigned flags,
...
static int prog_add_sport(struct prog *prog, const char *arg)
{
...
struct event_base *prog_eb(struct prog *prog)
{
    return prog->prog_eb;
}

int prog_connect(struct prog *prog, unsigned char *sess_resume, size_t sess_resume_len)
{
    struct service_port *sport;
...
void prog_process_conns(struct prog *prog)
{
    int diff;
    ...
    {
        if (diff < 0 || (unsigned)diff < prog->prog_settings.es_clock_granularity)
        {
            timeout.tv_sec = 0;
...
void prog_sport_cant_send(struct prog *prog, int fd)
{
    assert(!prog->prog_send);
