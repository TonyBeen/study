/*************************************************************************
    ...
    > Author: hsz
    > Brief:
    > Created Time: Tue 16 Nov 2021 10:40:59 AM CST
 ************************************************************************/
...
void icmp_pack(struct icmp *pIcmpHdr, int seq, int length)
{
    pIcmpHdr->icmp_type = ICMP_ECHO;
    pIcmpHdr->icmp_code = 0;
    pIcmpHdr->icmp_cksum = 0;
    pIcmpHdr->icmp_seq = seq;
    pIcmpHdr->icmp_id = pid & 0xffff;
    for (int i = 0; i < length; i++)
    {
        pIcmpHdr->icmp_data[i] = i;
    }

    pIcmpHdr->icmp_cksum = cal_chksum((unsigned short *)pIcmpHdr, length);
}
...
int icmp_unpack(char *buf, int len)
{
    ...
    // printf("icmp_type = %d, icmp_id = 0x%x\n", icmp->icmp_type, icmp->icmp_id);
...
int main(int argc, char *argv[])
{
    ...
    protocol = getprotobyname("ip"); // 获取协议类型ip
    if (protocol == NULL)
    {
    ...
    protocol = getprotobyname("tcp"); // 获取协议类型tcp
    if (protocol == NULL)
    {
        printf("Fail to getprotobyname!\n");
        return -1;
    ...
    protocol = getprotobyname("udp"); //获取协议类型tcp
    if(protocol == NULL)
    {
        printf("Fail to getprotobyname!\n");
        return -1;
    ...
    if (pthread_create(&send_id, NULL, (ThreadRoute)ping_send, NULL))
    {
        printf("Fail to create ping send thread!\n");
        return -1;
    }

    if (pthread_create(&recv_id, NULL, (ThreadRoute)ping_recv, NULL))
    {
