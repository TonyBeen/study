...
int main()
{
     ...
     *
     * symbol平均分到每个block中, 假设有17个symbol, 4个block, 不会是 [5, 5, 4, 3]这样分配而是 [5, 4, 4, 4]
     *
    ...
    uint16_t repair_symbol = 0;
    uint16_t redundancy_symbol = 4;
    for (uint8_t block = 0; block < blocks; ++block) {
        uint32_t sym_for_blk = RaptorQ_symbols(pEncoder, block);
        printf("Block: %i, sym_for_blk = %u\n", block, sym_for_blk);

        repair_symbol = sym_for_blk + redundancy_symbol; // 最少需要 sym_for_blk 个修复符号才能完整恢复数据
        ...
        repair_symbol = sym_for_blk + redundancy_symbol;
    }
    ...
    ++it; // 测试跳过第一个数据能否正常解码
    for (uint16_t i = 0; i < (repair_symbol - redundancy_symbol); ++i, ++it) {
        // printf("Id = %u\n", it->first);
